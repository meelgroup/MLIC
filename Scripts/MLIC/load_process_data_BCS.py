...
###########################################
#%% Define function
def load_process_data_BCS(train_file, test_file, colSep, rowHeader, colNames, col_y, fracPresent=0.9, colCateg=[], numThresh=9, valEq_y=None):
...
def dennis_example():
    ...
    # File parameters
    workDir = u'' # 'C:\\Users\\IBM_ADMIN\\Documents\\Model Interpretability\\Python'
    fileDir = './Data/' #u'\\Data\\'
    ...
    A, y = load_process_data_BCS(workDir + fileDir + fileName, colSep, rowHeader, colNames, col_y, colCateg=colCateg, valEq_y=2)
###########################################
def dmitry_example():
    ...
    col_y = colNames[-1]
    A, y = load_process_data_BCS(fname_data, colSep, rowHeader, colNames, col_y, valEq_y=2)
